...
# cron should cd to /home/emapex/python before running emarun.py
...
#
# /home/emapex/proc/emainfo has params and output directory names
#
...
def mklock(lockfile):
  if path.exists(lockfile):
    ...
    if path.exists(fn):
      print( datetime.utcnow().strftime('%Y-%b-%d %H:%M:%S'), 'UTC ',end='')
      print('mklock(): pid {0} is still running'.format(pid),end='')
