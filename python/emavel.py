...
if __name__ == '__main__':
  ...
  for arg in args:
    ...
    for hpid in hpids_proc:
      ...
      Pefp = np.interp(tefp, tctd, Pctd, left=np.nan, right=np.nan)
      Tefp = np.interp(tefp, tctd, Tctd, left=np.nan, right=np.nan)
      Sefp = np.interp(tefp, tctd, Sctd, left=np.nan, right=np.nan)

      if len(Pctd) > 1:
        Wctd = np.diff(Pctd) / np.diff(tctd)
        tW = (tctd[1:] + tctd[:-1]) * 0.5
        Wefp = np.interp(tefp,tW, Wctd, left=np.nan, right=np.nan)
      else:
        print('len(Pctd)=',len(Pctd),'too short to compute Wctd')
        count_no_W += 1
        continue
      ...
      c2e2 = -0.20
      if runid == '6624a':
        print('flipped alpha1 & alpha2 from standard')
        alpha1 = 1.95
        alpha2 = alpha1 - np.pi * 0.5
      else:
        # standard
        alpha2 = 1.95
