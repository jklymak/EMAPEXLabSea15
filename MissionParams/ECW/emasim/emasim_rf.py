...
# print('IridiumSecs=',secs,'carryover=',secs_carryover)
  return secs
...
def DescentProfTerminate():
  ...
  print(', pcw=',mission.PistonInitAscentLevel1)
  iPistonMoveAbs(mission.PistonInitAscentLevel1)
...
def AscentRun():
  ...
  if UpperPressureDetect (Pnow):
    ...
    if mission.CtdType == 4:
      tfin = tnow + Sbe41cpBinningSecs(Pdeepest)
      while tnow < tfin and tnow < endsecs:
        do_tstep()
      print_info('Sbe41cp_downloaded','\n')
    return
...
def AscentTerminate():
  ...
  iPistonMoveAbs(pcwant + mission.PistonInitialBuoyancyNudge)
  AirValveClose()
  StoreInFlash()
...
def plot_pvt():
  ...
  mplt.setp(xticklabels, visible=False)
...
def do_tstep():
  ...
  if air_valve_closed:
    ...
    if Pnow >= 0:
      Tnow = scipy.interpolate.interp1d(pts.P,pts.T)(Pnow)
      Snow = scipy.interpolate.interp1d(pts.P,pts.S)(Pnow)
  ...
  if tnow >= sho.tprev + sho.tstep - 0.1:
    sho.tprev = tnow
    sho.t.append(tnow)
    sho.P.append(Pnow)
